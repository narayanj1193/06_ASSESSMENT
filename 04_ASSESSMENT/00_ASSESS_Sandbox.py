...
def user_choice(question, valid_list):
    ...
    while True:
        ...
        for item in valid_list:
            if response == item[0] or response == item: # If response is the first letter of item,
...
def linear_generator(difficulty):
    ...
    if difficulty == "hard":
        ...
        y_intercept = random.uniform(-15, 15)
...
def answer_checker(eq1, eq2):
    ...
    try:
        # Convert the user's equation to a sympy expression
        user_expr = sp.sympify(eq2)
        # Simplify equation 1
        eq1_simplified = sp.sympify(eq1).simplify()
        # Simplify the user's equation
        user_expr_simplified = user_expr.simplify()

        # Compare the simplified equations for equality
        if eq1_simplified.equals(user_expr_simplified):
            return True
